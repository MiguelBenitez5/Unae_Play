...
def save_score(request, game_name:str, score:int) -> None:
    ...
    if max_score and max_score > score:
        ...
        global_rank, _ = GlobalRank.objects.update_or_create(user=user,defaults={'score': total})
        global_rank.save() 
...
Params:
    ...
    start_time (int o float): El tiempo de inicio de la partida en segundos\n
    max_score (int): Puntaje maximo del juego\n
    min_time (int): Tiempo record en que tomaria ganar la partida al jugador en segundos\n
    max_time (int): Tiempo maximo tolerable para finalizar la partida y recibir bonificacion de puntaje\n
